...
def train_individual_model(client_id, rounds=3):
    # Client 0: PTB-XL
    # Client 1: PTB-DB
    ...
    for round in range(1, rounds + 1):
        print(f"\nRound {round}")
        train(net, trainloader, optimizer, rounds=1, device=device)
...
def main():
    ...
    train_individual_model(client_id=0, rounds=3)
    
    # Running independent training for Client 1 (PTBDB)
    print("Loading PTB-DB (Client 1)")
    train_individual_model(client_id=1, rounds=3)
